...
function Keycloak (config) {
    ...
    kc.init = function (initOptions = {}) {
        ...
        function onLoad() {
            // Get Tide Config from Keycloak
            var getTideConfig = function () {
                console.log("hi");
                kc.tideConfig = {
                    tagAuthorization : "blabla"
                };
            }
            getTideConfig();
    ...
    kc.encrypt = async function(toEncrypt){
        ...
        if(!kc.tideConfig) throw 'Tide Config could not be found';

        toEncrypt.map(e => {
            if(!isObject(e)) throw 'All entries must be an object to encrypt';
            ...
            for (const tag of e.tags){
                if(typeof tag !== "string") throw "tags must be provided as an array of strings";
                var tagAccess = kc.resource_access[tag];
                if(!tagAccess) throw `'User has not been given any access to '${tag}'`;
                if(!tagAccess.roles.includes("encrypt")) throw `'User has not been given access to encrypt '${tag}'`;
            }
        });
    ...
    kc.decrypt = async function(options){
    ...
    function loadConfig() {
        ...
        function setupTideEndpoints(){
            kc.endpoints.tideTagAuthorization = function () {
                return getRealmUrl() + '/tide/tagAuthorization';
            }
            kc.endpoints.tideConfig = function () {
                return getRealmUrl() + '/tide/config';
            }
        }

        setupTideEndpoints(); // unconditional
