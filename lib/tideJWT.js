...
export async function verifyTideCloakToken(origin, token, AllowedRole) {
  ...
  try {
    ...
    if (!jwkData) {
      console.error(
        "[TideJWT] No JWKs found in tidecloak.json. Ensure client adapter is initialized. Switching to Trusting in TideCloak!"
      );
    }

    const JWKS = jwkData ? createLocalJWKSet(jwkData) : createRemoteJWKSet(new URL( thisIssuer + '/protocol/openid-connect/certs'));
